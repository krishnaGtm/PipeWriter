...
namespace PipeWriter
{
    ...
    public class DxfFile
    {
        ...
        public static string MergeDXF(string originalDXFFileNamne, string originalFileLocation,string destinationFileName, string destinationFileLocation, double originalStartX, double originalStartY, double originalEndX, double originalEndY, List<MergeContent> addedContents)
        {
            try
            {
                ...
                foreach (var _list in addedContents)
                {
                    ...
                    //rotation angle is subtracted from 360 degree, in svg rotation starts from x axis clockwise.
                    ...
                    foreach (var _dxfArc in addedDXF.Arcs)
                    {
                        ...
                        if (_list.Rotation > 0 && _list.Rotation < 360)
                        {
                            ...
                            if (_dxfArc.StartAngle < _dxfArc.EndAngle)
                                isClockwise = false;
                            ...
                            startPoint1.X = sofArcX;
                            startPoint1.Y = sofArcY;
                            var arcAngles = Coordinate.GetArcAngles(startPoint1, center, _dxfArc.Radius, Math.Abs(_dxfArc.StartAngle - _dxfArc.EndAngle), isClockwise);

                            _dxfArc.StartAngle = arcAngles[0];
                            _dxfArc.EndAngle = arcAngles[1];
                    ...
                    foreach (var _dxfEllipse in addedDXF.Ellipses)
                    {
                        ...
                        if (_list.Rotation > 0 && _list.Rotation < 360)
                        {
                            ...
                            center.X = cX;
                            center.Y = cY;
                            //_dxfEllipse.StartAngle = _dxfEllipse.StartAngle + (360 - rotationAngle);
                            //_dxfEllipse.EndAngle = _dxfEllipse.EndAngle + (360 - rotationAngle);
        ...
        /// <param name="xmlstring">String format of xml document</param>
